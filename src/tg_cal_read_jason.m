...
for nm=min_cir:max_cir
    ...
    dir_nm=strcat(dir_0,'cycle_',temp3,'\') % 输出文件夹名称
    ...
    file_num=temp(1);
    for n=1:file_num
        t1=str2double(namelist(n,17:19));
        if t1==pass_num % here is pass which you need to output data;009;147
            filepath=strcat(dir_nm,namelist(n,1:54));
            ...
            netcdf.close(nc)
            outfile=strcat('..\test\ja2_check\',namelist(n,13:19),'.dat');% 只取周期和pass编号
