...
function plot_hy2b_check_wet(pass_num,min_cir,max_cir,sat)
...
%         plot(latitude,points,'-',latitude,points_m,'+-r')
        xlabel('Latitude/°')
        ylabel('Wet troposhere delay from hy-2b AMR/mm')
...
figure (6)
if sat==3
    load ..\test\hy2_check\statistic.txt
end
w_m=statistic(:,2);
% &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
% 三倍中误差，剔除粗差
...
% &%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if sat==1
    save ..\test\hy2_check\hy2_wet_m_model.txt statistic -ascii
elseif sat==4
    save ..\test\hy2_check\hy2_wet_m_model.txt statistic -ascii
end
